...
class StripePaymentController extends Controller
{
    ...
    {
        $transactionID = 'trans-' . strtoupper(substr(str_shuffle('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 0, 10));
            ...
            case 'payment_intent.succeeded':
                ...
                if ($confirm && $confirm->status === 'succeeded') {
                    ...
                    if ($soldOut) {
                        ...
                        $removeFromCart = Cart::where('carlist_id', $soldOut->id)->first();
                        if ($removeFromCart) {
                            $removeFromCart->delete(); // Call delete as a method
                        } else {
                            Log::warning("Cart Not Found For Carlist ID: {$soldOut->id}");
                        }

                        $emptyCart = Cart::where('user_id', $updStatus->user_id);
                        if ($emptyCart->exists()) {
                            $emptyCart->delete(); // Delete all matching rows
                        } else {
                            Log::warning("Cart Not Found For User ID: {$updStatus->user_id}");
                        }
